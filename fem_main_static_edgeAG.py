...
m1, n1, m2, n2, ngauss = 1, 10, 1, 10, 30 
for e in range(nelem):
    if e == 1575:
        etypes[e] = (m1, n1, m2, n2, ngauss, 1) ## LHS outer
    elif e == 1576:
        etypes[e] = (m1, n1, m2, n2, ngauss, 2) ## LHS inner
    elif e == 1583:
        etypes[e] = (m1, n1, m2, n2, ngauss, 1) ## RHS inner
    elif e == 1584:
        etypes[e] = (m1, n1, m2, n2, ngauss, 2) ## RHS outer
    else:
        etypes[e] = (1, 1, 1, 1, 3, 1) 
...
script_dir = os.path.dirname(os.path.abspath(__file__))
# outdir = os.path.join(script_dir, f"{foroutput}_rigid_punch_({m1},{n1})_P=19.624e6_shift=0.003")
outdir = os.path.join(script_dir, f"2025_10_16_punch_results\Test2_{foroutput}({nx},{ny})_rigid_punch_({m1},{n1},{m2},{n2},{ngauss})_dz=-0.1")
os.makedirs(outdir, exist_ok=True)

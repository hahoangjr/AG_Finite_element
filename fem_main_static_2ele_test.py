...
m1, n1, m2, n2, ngauss, AGtype = 1, 5, 1, 2, 10, 2
for e in range(nelem):
    if e == 0:
        etypes[e] = (m1, n1, m2, n2, ngauss, AGtype)
    else:
...
# m1, n1, m2, n2, ngauss = 1, 10, 1, 10, 6 
# for e in range(nelem):
#     if e == 1575:
#         etypes[e] = (m1, n1, m2, n2, ngauss, 1)
#     elif e == 1576:
...
def is_symmetric(A, tol=1e-5):
    ...
    return A.shape[0] == A.shape[1] and np.allclose(A, A.T, rtol=0, atol=tol)
def plot_matrix(A, title=None, show_values=False, scientific=False, sigfigs=3, cmap="viridis"):
    A = np.asarray(A, float)
    m, n = A.shape
    fig, ax = plt.subplots(figsize=(4 + n*0.2, 3 + m*0.2), dpi=120)
    im = ax.imshow(A, cmap=cmap, aspect="auto")
    ax.set_xticks(range(n)); ax.set_yticks(range(m))
    if title: ax.set_title(title)
    plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)

    if show_values and m*n <= 2000:
        vmin, vmax = im.norm.vmin, im.norm.vmax
        mid = (vmin + vmax)/2
        fmt = (lambda x: f"{x:.{sigfigs}e}") if scientific else None
        for i in range(m):
            for j in range(n):
                x = A[i, j]
                if fmt is None:
                    # fixed unless exponent is large/small → then scientific
                    if x == 0: s = "0"
                    else:
                        e = int(np.floor(np.log10(abs(x))))
                        s = f"{x:.{max(0, sigfigs-1-e)}f}" if -3 <= e <= 3 else f"{x:.{sigfigs}e}"
                else:
                    s = fmt(x)
                ax.text(j, i, s, ha="center", va="center", color=("white" if x > mid else "black"), fontsize=8)
    fig.tight_layout(); return fig, ax    
###
print(is_symmetric(K))  # True    
plot_matrix(K, title="Stiffness matrix", scientific=True); plt.show()
...
script_dir = os.path.dirname(os.path.abspath(__file__))
# outdir = os.path.join(script_dir, f"{foroutput}_rigid_punch_({m1},{n1})_P=19.624e6_shift=0.003")
# outdir = os.path.join(script_dir, f"{foroutput}({nx},{ny})_rigid_punch_({m1},{n1},{ngauss})_dz=-0.1")
outdir = os.path.join(script_dir, f"2025_10_16_1element_test\{foroutput}({nx},{ny})_({m1},{n1},{m2},{n2},{ngauss},{AGtype})_prescribeNode6_dz={dz}")
os.makedirs(outdir, exist_ok=True)
...
#%% Export nodal displacement, nodal force, stiffness matrix
...
path = os.path.join(outdir, "stiffness_matrix.xlsx")  # outdir is a folder
pd.DataFrame(K).to_excel(path, sheet_name="stiffness matrix", index=False, header=False)
